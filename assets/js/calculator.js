(function () {
  ...
  var currentLocale = window.RaytonI18n ? window.RaytonI18n.getLocale() : 'uk';
  ...
  forms.forEach(function (form, index) {
    ...
    form.addEventListener('submit', function (event) {
      ...
      if (!results) { window.location.assign('calculator.html?' + params + '#solar-results'); return; }
      render(input);
  ...
  window.addEventListener('rayton:localechange', function (event) {
    currentLocale = copy[event.detail.locale] ? event.detail.locale : 'uk';
    if (currentInput) render(currentInput);
    forms.forEach(function (form) {
      var error = form.querySelector('.calc__error');
      if (error && !error.hidden) error.textContent = t('invalid');
    });
  });
})();
